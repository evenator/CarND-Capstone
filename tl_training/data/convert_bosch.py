...
def create_tf_example(example):
    ...
    for box in example['boxes']:
        ...
        #print("adding box")
        xmin=(float(box['x_min'] / width))
        xmax=(float(box['x_max'] / width))
        ymin=(float(box['y_min'] / height))
        ymax=(float(box['y_max'] / height))
        if (xmin < 0 or xmin > 1):
            return None    
        if (ymin < 0 or ymin > 1):
            return None    
...
def main(_):
    ...
    writer = tf.python_io.TFRecordWriter('bosch_train.record')
    # BOSCH
